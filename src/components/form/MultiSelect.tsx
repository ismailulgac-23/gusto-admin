...
const MultiSelect: React.FC<any> = ({
  ...
  const handleSelect = (option: OptionItem) => {
    ...
    let newSelectedOptions: OptionItem[];
  ...
  const removeOption = (option: OptionItem) => {
        ...
        <div className="relative flex flex-col items-center">
              ...
              <div className="flex flex-wrap flex-auto gap-2">
                {selectedOptions.length > 0 ? (
                  selectedOptions.map((option, index) => (
                    ...
                    >
                      <span className="flex-initial max-w-full">{option.label}</span>
          ...
          {isOpen && (
            <>
              ...
              <div
                className="absolute left-0 z-[9999] w-full overflow-y-auto bg-white rounded-lg shadow-lg top-full max-h-60 dark:bg-gray-800"
                  ...
                  {options.length > 0 ? (
                    options.map((option: any, index: any ) => (
                      <div key={index} className="w-full">
                        <div
                          className={`hover:bg-primary/5 w-full cursor-pointer border-b border-gray-200 dark:border-gray-700`}
                          onClick={() => handleSelect(option)}
                        >
                          <div
                            className={`relative flex w-full items-center p-2 pl-2 ${
                              isOptionSelected(option.value) ? "bg-primary/10" : ""
                            }`}
                          >
