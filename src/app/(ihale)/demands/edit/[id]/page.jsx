...
export default function EditDemand() {
    ...
    const fetchDemand = async () => {
        ...
        try {
            ...
            const demand = response.data.data || response.data;
            ...
            if (demand.countie) {
                try {
                    const parsedCounties = JSON.parse(demand.countie);
                    const cityNames = Object.keys(parsedCounties);
                    if (cityNames.length > 0) {
                        const cityName = cityNames[0];
                        const counties = parsedCounties[cityName];
            ...
            // If no city from countie, try to get from cities relation
            if (!selectedCityId && demand.cities && demand.cities.length > 0) {
                selectedCityId = demand.cities[0].cityId;
            }
            ...
            setFormData({
                ...
                latitude: demand.latitude?.toString() || "",
                longitude: demand.longitude?.toString() || "",
                budget: demand.budget?.toString() || "",
                images: demand.images || [],
                peopleCount: demand.peopleCount?.toString() || "",
                eventDate: demand.eventDate ? new Date(demand.eventDate).toISOString().split('T')[0] : "",
                eventTime: demand.eventTime || "",
                isUrgent: demand.isUrgent || false,
                deadline: demand.deadline || "",
                address: demand.address || "",
                questionResponses: demand.questionResponses || null,
            ...
            alert('Talep yüklenirken bir hata oluştu.');
            router.push('/demands');
        } finally {
            setIsFetching(false);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.patch(`/admin/demands/${id}`, data);
            router.push('/demands');
        } catch (error) {
            console.error('Error updating demand:', error);
            alert(error.response?.data?.message || error.message || 'Talep güncellenirken bir hata oluştu.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (isFetching) {
        return (
            <div>
                <PageBreadcrumb pageTitle="Talep Düzenle" />
        ...
        <div>
            ...
            <div className="space-y-6">
                <ComponentCard
                    ...
                    desc="Talep bilgilerini düzenlemek için aşağıdaki formu kullanın"
                >
