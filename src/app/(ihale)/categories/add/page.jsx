...
export default function AddCategory() {
    ...
    useEffect(() => {
        const fetchParentCategories = async () => {
            try {
                ...
                // Filter out categories that already have a parent (only show root categories as potential parents)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert(error.response?.data?.message || error.message || 'Kategori oluşturulurken bir hata oluştu.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div>
            <PageBreadcrumb pageTitle="Yeni Kategori Ekle" />
            <div className="space-y-6">
                <ComponentCard
                    title="Kategori Bilgileri"
                    desc="Yeni bir kategori oluşturmak için aşağıdaki formu doldurun"
                >
                                ...
                                >
                                    ...
                                    {parentCategories.map((category) => (
                                        <option key={category.id} value={category.id}>
                                            {category.icon && <Icon icon={category.icon} className="inline mr-1" />}
                                            {category.name}
                                        </option>
                                    ))}
                                </select>
